...
import { addNewOfferApi } from "API/offer.api";
...
export default function CreateOfferModal({
  ...
  const [value, setValue] = React.useState(offerNullMenuList[0]?.title);
  ...
  const addNewOffer = async (e) => {
    ...
    if (validate()) {
      try {
        const newOffer = await addNewOfferApi({
          name,
          discount,
          description,
          menu,
  ...
  const percentageOnChange = (event) => {
    ...
    let discountVal = (price_ * prct) / 100;
  ...
  const quantityThreshOnChange = (event) => {
              ...
              <GridItem xs={24} sm={24} md={18} style={{ width: "100%" }}>
                <Card>
                  <CardHeader color="primary">
                    <h4 className={classes.cardTitleWhite}>Create Offer</h4>
                    ...
                    <GridContainer>
                      ...
                      <GridItem xs={12} sm={12} md={4}>
                        <CustomInput
                          ...
                          onChange={quantityThreshOnChange}
