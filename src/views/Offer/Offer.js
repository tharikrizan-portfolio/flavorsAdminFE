...
import { getAllOffersApi } from "API/offer.api";
import { unCancelOfferApi } from "API/offer.api";
import { cancelOfferApi } from "API/offer.api";
...
export default function Offer() {
  ...
  const deleteOffer = async (offer) => {
    const deletedOffer = await deleteOfferApi({ _id: offer._id });
  ...
  const cancelOffer = async (offer) => {
    const cancelledOffer = await cancelOfferApi({ _id: offer._id });
  ...
  const unCancelOffer = async (offer) => {
    const unCancelledOffer = await unCancelOfferApi({
      _id: offer._id,
    });
    const newOfferList = offerList.map((singleOffer) => {
      if (singleOffer._id !== offer._id) {
